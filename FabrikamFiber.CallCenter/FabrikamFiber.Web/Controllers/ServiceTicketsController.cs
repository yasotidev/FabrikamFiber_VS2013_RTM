...
namespace FabrikamFiber.Web.Controllers
{
   ...
   public class ServiceTicketsController : Controller
   {
      ...
      public ViewResult Index()
      {
         return
            View(
               this.serviceTicketRepository.AllIncluding(
      ...
      public ViewResult Details(int id)
      {
         return
            View(
               this.serviceTicketRepository.FindIncluding(
                  id,
                  serviceticket => serviceticket.Customer,
                  serviceticket => serviceticket.CreatedBy,
                  serviceticket => serviceticket.AssignedTo));
      ...
      public ViewResult Schedule(int serviceTicketId, int employeeId, float startTime)
      {
         var viewModel = new ScheduleViewModel
         {
            ...
            ScheduleItems =
               this.scheduleItemRepository.AllIncluding(e => e.ServiceTicket)
      ...
      public ActionResult AssignSchedule(int serviceTicketId, int employeeId, float startTime)
      {
         ...
         var time = string.Format(
            "Mon 16 May {0:d2}:{1:d2} {2} 2011",
            ((int) startTime > 12 ? (int) startTime - 12 : (int) startTime) / 1,
            startTime % 1 == 0.5 ? 30 : 0,
            startTime < 12 ? "AM" : "PM");
         var startAt = DateTime.ParseExact(time, "ddd dd MMM h:mm tt yyyy", CultureInfo.InvariantCulture);
      ...
      public ActionResult Create(ServiceTicket serviceticket)
      {
         if (ModelState.IsValid)
         {
            ...
            if (createdBy != null) { serviceticket.CreatedByID = createdBy.ID; }
